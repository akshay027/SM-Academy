...
public class UpcomingSessionFragment extends Fragment {
    ...
    private ArrayList<UpcomingSession> upcomingSessionArrayList, searchArrayList;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        listviewdefaulter = (ListView) view.findViewById(R.id.lvUpcomingSession);
        ...
        upcomingSessionAdapter = new UpcomingSessionAdapter(getActivity(), upcomingSessionArrayList);
    ...
    private void filterSearch(String search) {
        try {
            ...
            for (int i = 0; i < searchArrayList.size(); i++) {
                UpcomingSession defaulterDatamodel = searchArrayList.get(i);
                    ...
                    public void success(LiveSessionResponse defaulterResponseModel, Response response) {
                        if (response.getStatus() == Constants.SUCCESS) {
                            try {
                                ...
                                upcomingSessionArrayList.addAll(defaulterResponseModel.getUpcomingLiveSessions());
                                listviewdefaulter.setAdapter(upcomingSessionAdapter);
                                upcomingSessionAdapter.notifyDataSetChanged();
                                Log.e("upcomingSession", " --- " + upcomingSessionArrayList);
                                if (upcomingSessionArrayList.size() <= 0) {
                                    listviewdefaulter.setVisibility(View.GONE);
                                    searchView.setVisibility(View.GONE);
                                    tv_nodata.setVisibility(View.VISIBLE);
                                    tv_nodata.setText("You do not have any upcoming sessions");
                                } else {
